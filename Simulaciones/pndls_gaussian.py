...
if __name__ == '__main__':
    ...
    for i in range(1):
        for j in range(1):
            ...
            ####### DINAMICA #########
            campos_finales = RK4_PDE(eq, campos, bordes, dx, dt, Nx, Nt, control=1, alpha=alpha, beta=beta, gamma=gamma, mu=mu, nu=nu, forzamiento=fuentes)
            ...
            sim_file = nombre_pndls_estandar(gamma, nu)
            os.makedirs(simulation_data_path + sim_file)
            guardar_txt([x_grid, t_ligero, campo_ligeros[0], campo_ligeros[1]], simulation_data_path, sim_file)
